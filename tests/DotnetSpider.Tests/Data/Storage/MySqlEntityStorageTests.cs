...
namespace DotnetSpider.Tests.Data.Storage
{
    public class MySqlEntityStorageTests : TestBase
    {
        ...
        {
            return new MySqlEntityStorage(type, _connectionString);
        }
        ...
        public async Task CreateTable()
        {
            ...
            {
                // 如果实体的 Schema 没有配置表名，则使用类名
                await conn.ExecuteAsync("drop table if exists `test`.`createtable`;");
                var services = SpiderFactory.CreateScopeServiceProvider();
                var storage = CreateStorage(StorageType.Insert);
                var dfc = new DataFlowContext(null, services);
                ...
                dfc.AddParseItem(typeName, items);
                await storage.HandleAsync(dfc);
                var list = (await conn.QueryAsync<CreateTableEntity3>("SELECT * FROM `test`.`createtable`")).ToList();
                Assert.Single(list);
                entity = list.First();
                Assert.Equal("xxx", entity.Str1);
                Assert.Equal("yyy", entity.Str2);
        ...
        public async Task CreateTableAutoIncPrimary()
        {
            ...
            {
                // 如果实体的 Schema 没有配置表名，则使用类名
                await conn.ExecuteAsync("drop table if exists `test`.`createtableautoincprimay`;");
                var services = SpiderFactory.CreateScopeServiceProvider();
                var storage = CreateStorage(StorageType.Insert);
                var dfc = new DataFlowContext(null, services);
                ...
                await storage.HandleAsync(dfc);
                var list =
                    (await conn.QueryAsync<CreateTableEntity5>("SELECT * FROM `test`.`createtableautoincprimay`"))
                    .ToList();
        ...
        public async Task UpdatePartColumns()
        {
            ...
            {
                // 如果实体的 Schema 没有配置表名，则使用类名
                await conn.ExecuteAsync("drop table if exists `test`.`updatepartcolumns`;");
                var services = SpiderFactory.CreateScopeServiceProvider();
                var storage = CreateStorage(StorageType.InsertIgnoreDuplicate);
                var dfc = new DataFlowContext(null, services);
                ...
                var list = (await conn.QueryAsync<CreateTableEntity6>("SELECT * FROM `test`.`updatepartcolumns`"))
                    .ToList();
                Assert.Single(list);
                entity = list.First();
                Assert.Equal("TTT", entity.Str1);
        ...
        public async Task IgnoreCase()
        {
            ...
            {
                // 如果实体的 Schema 没有配置表名，则使用类名
                await conn.ExecuteAsync("drop table if exists `test`.`IgnoreCase`;");
                var services = SpiderFactory.CreateScopeServiceProvider();
                ...
                dfc.AddParseItem(typeName, items);
                await storage.HandleAsync(dfc);
                var list = (await conn.QueryAsync<CreateTableEntity7>("SELECT * FROM `test`.`IgnoreCase`")).ToList();
                Assert.Single(list);
                entity = list.First();
                Assert.Equal("xxx", entity.Str1);
                Assert.Equal("yyy", entity.Str2);
