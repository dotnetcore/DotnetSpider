...
/// </summary>
public class PostgreSqlEntityStorage : RDEntityStorageBase
{
    /// <summary>
    /// 根据配置返回存储器¬
    /// </summary>
        ...
        SqlStatements sqlStatements)
    {
        if (!string.IsNullOrWhiteSpace(sqlStatements.CreateDatabaseSql))
        {
            ...
            {
                if (e.Message != $"42P04: database {sqlStatements.DatabaseNameSql} already exists")
                {
                    throw;
                }
            }
    ...
    protected override SqlStatements GenerateSqlStatements(TableMetadata tableMetadata)
    {
        var sqlStatements = new SqlStatements
        {
            ...
            DatabaseNameSql = string.IsNullOrWhiteSpace(tableMetadata.Schema.Database)
                ? ""
