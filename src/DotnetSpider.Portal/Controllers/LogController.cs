	...
	public class LogController : Controller
	{
		public IActionResult List(string identity, int? page = 1, int? pageSize = 15)
		{
			using (MySqlConnection conn = new MySqlConnection(Startup.Configuration.GetSection("ConnectionStrings")["MySqlConnectString"]))
			{
				...
				List<LogInfo> list = conn.Query<LogInfo>($"SELECT * FROM nlog.log where identity='{identity}' ORDER BY id DESC LIMIT {(no - 1) * size},{size}").ToList();
				var totalCount = conn.Query<CountResult>($"SELECT COUNT(*) AS Count FROM nlog.log where identity='{identity}'").First();
