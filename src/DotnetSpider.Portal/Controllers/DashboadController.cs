...
namespace DotnetSpider.Portal.Controllers
{
	public class DashboadController : Controller
	{
		...
		public IActionResult Task()
		{
			...
			{
				List<TaskStatus> list = conn.Query<TaskStatus>($"SELECT * FROM nlog.status WHERE `logged`>='{DateTime.Now.ToString("yyyy-MM-dd")}'").ToList();
				...
				ViewBag.ExceptionPercent = (int)(ViewBag.ExceptionCount / (float)ViewBag.TotalCount * 100);
				ViewBag.FinishedCount = list.Count(t => t.Status == TaskStatus.StatusCode.Finished);
				ViewBag.FinishedPercent = (int)(ViewBag.FinishedCount / (float)ViewBag.TotalCount * 100);
				ViewBag.RunningCount = list.Count - ViewBag.ExceptionCount - ViewBag.FinishedCount;
				ViewBag.RunningPercent = (int)(ViewBag.RunningCount / (float)ViewBag.TotalCount * 100);
				return View();
			}
		}
		...
		public IActionResult Log()
		{
			using (MySqlConnection conn = new MySqlConnection(Startup.Configuration.GetSection("ConnectionStrings")["MySqlConnectString"]))
			{
				ViewBag.LogCount = conn.Query<CountResult>("SELECT COUNT(*) AS Count FROM nlog.log;").First().Count / 10000 + "万";
				return View();
			}
		}
	}
