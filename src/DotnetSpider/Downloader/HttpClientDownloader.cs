        ...
        protected override async Task<Response> ImplDownloadAsync(Request request)
        {
            ...
            for (int i = 0; i < RetryTime; ++i)
            {
                ...
                try
                {
                    ...
                    httpResponseMessage = await Singleton<NetworkCenter>.Instance.Execute(async () =>
                    ...
                    if (Regex.IsMatch(response.RawText, request.ChangeIpPattern))
                    {
                        if (UseProxy)
                        {
                        ...
                        {
                            // 不支持切换 IP
                            if (Singleton<NetworkCenter>.Instance == null ||
                                !Singleton<NetworkCenter>.Instance.SupportAdsl)
                            {
                                response.Success = false;
                            ...
                            {
                                Singleton<NetworkCenter>.Instance.Redial();
                            }
                        }
                    }
                    else
