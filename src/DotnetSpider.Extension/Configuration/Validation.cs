...
namespace DotnetSpider.Extension.Configuration
{
	public class Validations
	{
		...
		{
			if (string.IsNullOrEmpty(EmailFrom) || string.IsNullOrEmpty(ConnectString) || string.IsNullOrEmpty(EmailTo) || string.IsNullOrEmpty(EmailPassword) || string.IsNullOrEmpty(EmailSmtpServer) || string.IsNullOrEmpty(EmailUser))
			{
				return null;
			}
	...
	public class EqualValidation : Validation
	{
		...
		{
			return new DotnetSpider.Validation.EqualValidation(conn, Sql, Arguments, Description, Level);
		}
	}
	...
	public class RangeValidation : Validation
	{
		...
		public override IValidation GetValidation(DbConnection conn)
		{
			return new DotnetSpider.Validation.RangeValidation(conn, Sql, Arguments, Description, Level);
		}
	}
}